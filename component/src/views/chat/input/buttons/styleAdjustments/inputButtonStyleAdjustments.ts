...
export class InputButtonStyleAdjustments {
  private static adjustInputPadding(textInputEl: HTMLElement, pToBs: PositionToButtons) {
    if (pToBs[INSIDE_START].length > 0) {
      textInputEl.classList.add('text-input-inner-start-adjustment');
    }
    if (pToBs[INSIDE_END].length > 0) {
      textInputEl.classList.add('text-input-inner-end-adjustment');
    }
  }
  ...
  private static adjustForOutsideButton(containers: ButtonContainersT, fileAtt: HTMLElement, pToBs: PositionToButtons) {
    if (pToBs[OUTSIDE_END].length === 0 && pToBs[OUTSIDE_START].length > 0) {
      containers[0].classList.add(INPUT_OUTSIDE_START_SMALL_ADJUSTMENT_CLASS);
      fileAtt.classList.add(INPUT_OUTSIDE_START_SMALL_ADJUSTMENT_CLASS);
    } else if (pToBs[OUTSIDE_START].length === 0 && pToBs[OUTSIDE_END].length > 0) {
      containers[3].classList.add(INPUT_OUTSIDE_END_SMALL_ADJUSTMENT_CLASS);
      fileAtt.classList.add(INPUT_OUTSIDE_END_SMALL_ADJUSTMENT_CLASS);
    }
  }
  ...
  private static adjustOutsideSubmit(containers: ButtonContainersT, fileAtt: HTMLElement, pToBs: PositionToButtons) {
    ...
    if (pToBs[OUTSIDE_END].length === 0 && pToBs[OUTSIDE_START].length > 0) {
      containers[0].classList.add(INPUT_OUTSIDE_START_ADJUSTMENT_CLASS);
      fileAtt.classList.add(INPUT_OUTSIDE_START_ADJUSTMENT_CLASS);
      return pToBs[OUTSIDE_START].map((element) => element.button.elementRef.classList.add('submit-button-enlarged'));
    } else if (pToBs[OUTSIDE_START].length === 0 && pToBs[OUTSIDE_END].length > 0) {
      containers[3].classList.add(INPUT_OUTSIDE_END_ADJUSTMENT_CLASS);
      fileAtt.classList.add(INPUT_OUTSIDE_END_ADJUSTMENT_CLASS);
      return pToBs[OUTSIDE_END].map((element) => element.button.elementRef.classList.add('submit-button-enlarged'));
    }
    return undefined;
  }
