...
export class BrowserStorage {
  ...
  public addInputText(inputText: string) {
    ...
    const item = localStorage.getItem(this.storageKey);
    const parsedItem = item ? JSON.parse(item) : undefined;
    this.set(parsedItem?.messages || [], inputText, this.trackScrollHeight ? parsedItem?.scrollHeight : undefined);
  }
  ...
  public addScrollHeight(scrollHeight: number) {
    ...
    const item = localStorage.getItem(this.storageKey);
    const parsedItem = item ? JSON.parse(item) : undefined;
    this.set(parsedItem?.messages || [], this.trackInputText ? parsedItem?.inputText : undefined, scrollHeight);
  }
