...
<ComponentContainer>
  <DeepChatBrowser
    ...
    submitButtonStyles={{
      ...
      loading: {
        container: {default: {backgroundColor: '#afafaf'}},
        text: {
          content: 'Wait',
        },
        svg: {
...
Messages are grouped by the role their role. The styling is set via the `deep-chat-top-message`, `deep-chat-middle-message` and `deep-chat-bottom-message` classes
using the [`auxiliaryStyle`](/docs/styles#auxiliaryStyle) property.
