...
- Type: `true` | {`ms?: number`, `stop?: boolean`}
- Default: _{ms: 2000, stop: true}_

Configuration to automatically submit the input message after a period of silence. <br />
`ms` is the number of milliseconds of silence to wait before the message is submitted. <br />
`stop` toggles whether the recording should stop after the message is submitted. <br />
...
```html
<deep-chat speechToText='{"submitAfterSilence": {"ms": 3000, "stop": false}}'></deep-chat>
```
